...
export async function getPipelineValue(): Promise<PipelineValue> {
  const { data, error } = await supabase.from('v_kpi_pipeline_value').select('*').single();
...
export async function getExpectedRevenue(): Promise<ExpectedRevenue> {
  const { data, error } = await supabase.from('v_kpi_expected_revenue').select('*').single();
...
export async function getLeadsBySource30d(): Promise<LeadsBySource[]> {
  const { data, error } = await supabase.from('v_kpi_leads_by_source_30d').select('*');
...
export async function getTasksDue(): Promise<TasksDue> {
  const { data, error } = await supabase.from('v_kpi_tasks_due').select('*').single();
